...
A [timoni.sh](http://timoni.sh) module for deploying cilium-network-policy to Kubernetes clusters.
...
```shell
timoni -n default apply cilium-network-policy oci://<container-registry-url>
```
...
```shell
timoni -n default delete cilium-network-policy
```
